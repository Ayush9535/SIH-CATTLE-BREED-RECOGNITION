// ... (This tool call will handle the full revert logic)
...
import { detectBreed, detectMultipleBreeds, initializeModel, checkIsBovine } from '../../src/services/tflite';
...
export default function UploadScreen(): React.JSX.Element {
    ...
    const requestPermissions = async () => {
        ...
        if (status !== 'granted') {
            Alert.alert(t('uploadExt.permissionDenied'), t('uploadExt.needCameraRoll'));
            return false;
        }
        return true;
    };
    ...
    const requestCameraPermissions = async () => {
        ...
        if (status !== 'granted') {
            Alert.alert(t('uploadExt.permissionDenied'), t('uploadExt.needCamera'));
            return false;
        }
        return true;
    };
    ...
    const pickImageFromCamera = async () => {
        if (selectedImages.length >= 3) {
            Alert.alert(t('uploadExt.limitReached'), t('uploadExt.limit3'));
            return;
        }
        ...
        if (!result.canceled && result.assets[0]) {
            ...
            if (image.fileSize) {
                ...
                if (!sizeCheck.isValid) {
                    Alert.alert(t('uploadExt.imageTooLarge'), sizeCheck.error + '\n\n' + t('uploadExt.chooseSmaller'));
                    return;
                }
            }
            ...
            if (image.mimeType) {
                ...
                if (!typeCheck.isValid) {
                    Alert.alert(t('uploadExt.invalidImageType'), typeCheck.error + '\n\n' + t('uploadExt.selectValidTypes'));
                    return;
                }
            }
            ...
            // Bovine Verification
            setIsAnalyzing(true);
            setUploadProgress(t('upload.processingImage') || 'Verifying image...');
            const isBovine = await checkIsBovine(image.uri);
            setIsAnalyzing(false);
            setUploadProgress('');

            if (!isBovine) {
                Alert.alert(
                    t('uploadExt.validBovineTitle'),
                    t('uploadExt.validBovineMsg')
                );
                return;
            }
    ...
    const pickImageFromGallery = async () => {
        ...
        if (!result.canceled && result.assets[0]) {
            ...
            // Bovine Verification
            setIsAnalyzing(true);
            setUploadProgress(t('upload.verifying') || 'Verifying image...');
            const isBovine = await checkIsBovine(image.uri);
            setIsAnalyzing(false);
            setUploadProgress('');
    ...
    const analyzeImage = async () => {
        if (selectedImages.length === 0) {
            Alert.alert(t('upload.noImageSelected'), t('upload.selectImageFirst'));
            return;
        }
        ...
        try {
            ...
            setUploadProgress(t('upload.analyzing'));
            ...
            const breedData = {
                ...
                description: t('uploadExt.detectedWithConfidence', { confidence: detection.confidence.toFixed(2) }),
                characteristics: [
                    t('uploadExt.offlineCharacteristic1'),
                    t('uploadExt.offlineCharacteristic2'),
                ],
                careTips: [
                    t('uploadExt.offlineCareTip1'),
                    t('uploadExt.offlineCareTip2'),
                    t('uploadExt.offlineCareTip3'),
                ],
            };
            ...
            Alert.alert(t('common.error'), error.message || t('uploadExt.failedToAnalyze'));
        ...
        >
            <View style={styles.content}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={styles.subtitle}>{t('uploadExt.subtitle')}</Text>
                </View>
                ...
                <View style={styles.gridContainer}>
                    ...
                    {selectedImages.length < 3 && (
                        ...
                        >
                            ...
                            <Text style={styles.addMoreText}>{t('uploadExt.addPhoto')}</Text>
                        </TouchableOpacity>
                    )}
                </View>
                ...
                {selectedImages.length > 0 && (
                    ...
                    >
                        {isAnalyzing ? (
                            ...
                            <Text style={styles.analyzeButtonText}>🔍 {t('uploadExt.analyzeBreed')}</Text>
                        )}
                    </TouchableOpacity>
                )}
