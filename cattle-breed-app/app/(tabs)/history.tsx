                ...
                onPress={() => {
                    ...
                    router.push({
                        ...
                        params: {
                            ...
                            // Let's pass what we have.
                            description: t('history.detectedOn', { breed: topPrediction.breed, date: formatDate(item.timestamp) }),
                        }
                    } as any);
                }}
            >
                ...
                <View style={styles.cardContent}>
                    <View style={styles.headerRow}>
                        <Text style={styles.breedName}>{t(`breeds.${toCamelCase(topPrediction.breed)}`) || topPrediction.breed}</Text>
                        <Text style={styles.confidence}>{(topPrediction.confidence).toFixed(2)}%</Text>
                    </View>
                    ...
                    <View style={styles.syncContainer}>
                        <Text style={[
                            ...
                            {item.isSynced ? `☁️ ${t('history.synced')}` : `⏳ ${t('history.pendingSync')}`}
                        </Text>
                    </View>
                </View>
            </TouchableOpacity>
        ...
        return (
            <View style={styles.container}>
                <View style={styles.header}>
                    <Text style={styles.title}>{t('history.scanHistory')}</Text>
                </View>
                <View style={styles.emptyContainer}>
                    <Text style={styles.emptyIcon}>🔒</Text>
                    <Text style={styles.emptyText}>{t('history.loginRequired')}</Text>
                    <Text style={styles.emptySubtext}>{t('history.loginRequiredDesc')}</Text>
                    <TouchableOpacity
                        style={styles.loginButton}
                        onPress={() => router.push('/login' as any)}
                    >
                        <Text style={styles.loginButtonText}>{t('history.loginNow')}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        );
        ...
        <View style={styles.container}>
            ...
            {user?.role === 'flw' && (
                <View style={styles.tabContainer}>
                    ...
                    >
                        <Text style={[styles.tabText, activeTab === 'predictions' && styles.activeTabText]}>{t('history.predictions')}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                        style={[styles.tab, activeTab === 'registrations' && styles.activeTab]}
                        onPress={() => setActiveTab('registrations')}
                    >
                        <Text style={[styles.tabText, activeTab === 'registrations' && styles.activeTabText]}>{t('history.registrations')}</Text>
                    </TouchableOpacity>
                </View>
            )}
            ...
            {activeTab === 'predictions' ? (
                history.length === 0 ? (
                    <View style={styles.emptyContainer}>
                        ...
                        <Text style={styles.emptySubtext}>{t('history.noScansDesc')}</Text>
                    </View>
                ) : (
                    <FlatList
                        data={history}
                ...
                registrations.length === 0 ? (
                    <View style={styles.emptyContainer}>
                        <Text style={styles.emptyText}>{t('history.noRegistrations')}</Text>
                        <Text style={styles.emptySubtext}>{t('history.noRegistrationsDesc')}</Text>
                    </View>
                ) : (
                    <FlatList
                        data={registrations}
